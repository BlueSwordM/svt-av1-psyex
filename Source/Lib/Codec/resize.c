...
static uint8_t get_superres_denom_for_qindex(SequenceControlSet *scs, PictureParentControlSet *pcs, int qindex,
    ...
    const double energy_by_q2_thresh = scs->static_config.tune == TUNE_VQSSIM ? 0.01 : get_energy_by_q2_thresh(&scs->enc_ctx->rc, update_type);
    ...
    case SUPERRES_AUTO: {
        ...
        if (q <= qthresh) {
            ...
            if (sr_search_type == SUPERRES_AUTO_SOLO) {
                ...
                int32_t update_type = svt_aom_get_frame_update_type(scs, pcs);
                if (update_type == SVT_AV1_KF_UPDATE || (update_type == SVT_AV1_ARF_UPDATE && scs->static_config.tune != TUNE_VQSSIM)) {
