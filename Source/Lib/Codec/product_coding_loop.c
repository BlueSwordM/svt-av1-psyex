...
static void fast_loop_core_light_pd0(ModeDecisionCandidateBuffer *cand_bf, PictureControlSet *pcs,
    ...
    if (ctx->lpd0_ctrls.pd0_level == VERY_LIGHT_PD0) {
        ...
        if (rtc_tune && ctx->mds0_ctrls.mds0_dist_type == VAR) {
    ...
    if (is_inter_singleref_mode(cand->block_mi.mode)) {
        ...
        if (is_inter_mode(cand_bf->cand->block_mi.mode) && is_obmc_allowed &&
            ...
            if (ctx->mds0_ctrls.mds0_dist_type == SSD) {
                ...
                cand_bf->luma_fast_dist = luma_fast_dist = spatial_full_dist_type_fun(input_pic->buffer_y,
                                                                                      input_origin_index,
                                                                                      input_pic->stride_y,
                                                                                      pred->buffer_y,
...
void fast_loop_core(ModeDecisionCandidateBuffer *cand_bf, PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    if (ctx->mds0_ctrls.pruning_method_th && ctx->pd_pass == PD_PASS_1) {
        if (ctx->mds0_ctrls.pruning_method_th != (uint8_t)~0 &&
            ...
            if (ctx->mds0_ctrls.per_class_dist_to_cost_th[cand_bf->cand->cand_class] != (uint16_t)~0 &&
                ...
                const uint64_t distortion_cost = RDCOST(lambda, 0, luma_fast_dist);
                if ((100 *
    ...
    if (ctx->shut_fast_rate) {
        ...
        *(cand_bf->fast_cost) = av1_product_fast_cost_func_table[is_inter_mode(cand->block_mi.mode)](
            pcs, ctx, cand_bf, lambda, luma_fast_dist);
    }
    cand_bf->valid_luma_pred = 1;
...
static void md_stage_0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    for (uint8_t itr = 0; itr < tot_itr; itr++) {
        for (uint32_t cand_idx = 0; cand_idx < fast_cand_count; cand_idx++) {
            ...
            if (pcs->scs->static_config.tune == TUNE_VQSSIM && pcs->ppcs->slice_type == B_SLICE) { // Only somewhat reverse the uni psy bias on B-Frames, prevents sharp / squiggling(lack of a better term?) artifacting
...
static void md_encode_block_light_pd1(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
    ...
    } else if (pcs->ppcs->scs->static_config.tune == TUNE_SSIM || pcs->ppcs->scs->static_config.tune == TUNE_VQSSIM) {
...
static void md_encode_block(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
    ...
    // 3rd Full-Loop
    ctx->md_stage        = MD_STAGE_3;
    ctx->tune_ssim_level = (pcs->scs->static_config.tune == TUNE_SSIM || pcs->ppcs->scs->static_config.tune == TUNE_VQSSIM) && (ctx->pd_pass == PD_PASS_1) ? SSIM_LVL_1 : SSIM_LVL_0;
