...
static void aom_av1_set_mb_ssim_rdmult_scaling(PictureParentControlSet *pcs) {
    ...
    for (int row = 0; row < num_rows; ++row) {
        for (int col = 0; col < num_cols; ++col) {
            ...
            if (pcs->scs->static_config.tune == TUNE_VQSSIM) {
                // Loop through each 16x16 block.
    ...
    } else { // Do superblock-based adjustment if we're on Tune 4 (VQ-SSIM)
...
void *svt_aom_source_based_operations_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        if (scs->static_config.tune == TUNE_SSIM || scs->static_config.tune == TUNE_VQSSIM) {
